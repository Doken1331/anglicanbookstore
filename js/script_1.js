...
jQuery(document).ready(function($) {
    ...
    if (o.length && !o.hasClass('kblocktable')) {
        function createTables(element) { element.each(function() { var el = $(this); var elDl = $("<dl/>", { "class": "table" }).insertAfter(el);
                el.find('th').each(function(i) { elDl.append('<dt>' + $(this).html() + '</dt>');
                    el.find('tbody tr').each(function() { elDl.append('<dd>' + $(this).find('td:nth-child(' + (i + 1) + ')').html() + '</dd>') }) }) }) }
        $(document).ready(createTables($('table'))) }
    $(':header').not('[class*=heading-style-]').not('.alert-heading').each(function() { $(this).addClass('heading-style-' + $(this).prop('tagName').replace(/^\D+/g, '')) });
    $('[class*="icon-"]').each(function() { $(this).addClass($(this).attr('class').replace(/icon-/i, "fa fa-")) });

    function setOffset() { $('.item__module,.item,.row-container,.categories-module,.archive-module,.tagspopular,.mod-login__aside,.form-vertical,.mod-search__aside,.contact_details_item,.contact_misc,.contact_form,.contact_map,h1,h2,h3,h4,h5,h6').not('.visible-first').each(function(i) { var y = $(this),
                v = y.offset().top,
                w = $(window),
                x = w.scrollTop(),
                a = v - x < w.height(),
                z = v - x > -y.height(); if (a && z) { y.addClass('visible-first'); if ($('[class*="progress"]').length) { if (y.find('>.item_content [class*="progress"]').length != 0 && !y.hasClass('animated')) { animateProgress(y) } } } }) }
    setOffset();
    $(window).bind("resize scroll", $.throttle(50, setOffset));

    function animateProgress(el) { if (el.find('.radial-progress').length != 0 && el.find('.radial-progress').hasClass('draw')) { $.each(el.find('.radial-progress'), function() { var progress = $(this).data('progress'); var circle = new ProgressBar.Circle('.radial-progress.draw', { color: '#000', trailColor: '#000', fill: '#000', strokeWidth: 10, trailWidth: 0, duration: 2000, easing: 'bounce', text: { value: '0' }, step: function(state, bar) { bar.setText((bar.value() * 100).toFixed(0)) } });
                circle.animate(progress / 100, function() {});
                $(this).removeClass('draw') }) } else { el.addClass('animated');
            el.find('.text').each(function() { var progress = $(this); var percentage = $(this).closest('[class*="progress"]').attr('data-progress');
                $({ countNum: 0 }).animate({ countNum: percentage }, { duration: 2000, easing: 'linear', step: function() { progress.text(Math.ceil(this.countNum) + '%') } }) }) } }
});
