    ...
    function setOffset() {
        $('.item__module,.item,.row-container,.categories-module,.archive-module,.tagspopular,.mod-login__aside,.form-vertical,.mod-search__aside,.contact_details_item,.contact_misc,.contact_form,.contact_map,h1,h2,h3,h4,h5,h6').not('.visible-first').each(function(i) {
            var y = $(this),
                ...
                a = v - x < w.height(),
                z = v - x > -y.height();
            if (a && z) { y.addClass('visible-first'); if ($('[class*="progress"]').length) { if (y.find('>.item_content [class*="progress"]').length != 0 && !y.hasClass('animated')) { animateProgress(y) } } }
        })
    }
    setOffset();
    ...
    function animateProgress(el) {
        if (el.find('.radial-progress').length != 0 && el.find('.radial-progress').hasClass('draw')) {
            $.each(el.find('.radial-progress'), function() {
                ...
                var circle = new ProgressBar.Circle('.radial-progress.draw', { color: '#000', trailColor: '#000', fill: '#000', strokeWidth: 10, trailWidth: 0, duration: 2000, easing: 'bounce', text: { value: '0' }, step: function(state, bar) { bar.setText((bar.value() * 100).toFixed(0)) } });
